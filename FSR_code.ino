...
// Pin assignments
const int FSR_COUNT = 1;

int OUT = 12;          // OUTPUT pin
int LED = 13;          // LED pin
int A[1]  = {1}; //,2,3};  // Analog pin numbers
int AD[1] = {2}; //,4,3};  // Digital pin numbers correlating to the physical analog pins

// Various variables used for state and tracking calculations
unsigned long PRECALIBRATION=0;
unsigned long SEED = 40;
float VALUE[FSR_COUNT];
float AVERAGE[FSR_COUNT];
float NOISE_LEVEL[FSR_COUNT]; 
float TRIGGER_LEVEL[FSR_COUNT];
float RECOVERY_LEVEL[FSR_COUNT];
unsigned long TALLY[FSR_COUNT];
float TOTAL[FSR_COUNT];
unsigned long STATE[FSR_COUNT];
unsigned long TRIGGER_ADJUST[FSR_COUNT];
unsigned long RECOVERY_ADJUST[FSR_COUNT];
long scratch;
...
void setup() {
  ...
  int idx;
  for (idx = 0; idx < FSR_COUNT; idx++) {
    VALUE[idx] = 0;
    AVERAGE[idx] = 0;
    NOISE_LEVEL[idx] = 8;
    TRIGGER_LEVEL[idx] = 0;
    RECOVERY_LEVEL[idx] = 0;
    TALLY[idx] = 0;
    TOTAL[idx] = 0;
    STATE[idx] = 0;
    TRIGGER_ADJUST[idx] = 150;
    RECOVERY_ADJUST[idx] = 75;
  }
  ...
  for(scratch=0; scratch<FSR_COUNT ; scratch++) {
...
void do_real_calibration()
{
  ...
  for(idx=0; idx<FSR_COUNT ; idx++ ) {
    ...
    //Serial.print(AVERAGE[idx]);
    //Serial.print("  Noise:");
    //Serial.print(NOISE_LEVEL[idx]);
    //Serial.print("  Trigger:");
    //Serial.print(TRIGGER_LEVEL[idx]);
    //Serial.print("  Recovery:");
    //Serial.println(RECOVERY_LEVEL[idx]);
  };
  PRECALIBRATION++;
};
